 ...
 *
 * @module pdf/pdf.ts-test/ink_editor.i.cy
 * @license Apache-2.0
 ******************************************************************************/
...
describe("Ink Editor", () => {
  describe("Basic operations", () => {
    ...
    it.skip("must draw, undo a deletion and check that the editors are not selected", () => {
      // await Promise.all(
    ...
    it.skip("must draw, undo/redo and check that the editor don't move", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await clearAll(page);
  ...
  describe("with a rotated pdf", () => {
    before(() => {
      cy.visit(`/${P_html}?${queriesFrom("issue16278.pdf")}`);
    });

    it.skip("must draw something", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await switchToInk(page);
  ...
  describe("Invisible layers must be disabled", () => {
    ...
    it.skip("must check that the editor layer is disabled", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await switchToInk(page);
  ...
  describe("Ink editor must be committed when blurred", () => {
    ...
    it.skip("must check that the ink editor is committed", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await switchToInk(page);
  ...
  describe("Undo a draw", () => {
    ...
    it.skip("must check that a draw can be undone", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await switchToInk(page);
  ...
  describe("Draw several times in the same editor", () => {
    ...
    it.skip("must check that we can draw several times on the same canvas", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await switchToInk(page);
      //     const rect = await getRect(page, ".annotationEditorLayer");
