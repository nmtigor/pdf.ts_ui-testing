 ...
 *
 * @module pdf/pdf.ts-test/find.i.cy
 * @license Apache-2.0
 ******************************************************************************/
...
describe("find bar", () => {
  describe("highlight all", () => {
    before(() => {
      cy.visit(`/${P_html}?${queriesFrom("find_all.pdf", 100)}`);
    });

    it.skip("must highlight text in the right position", () => {
      // await Promise.all(
  ...
  describe("highlight all (XFA)", () => {
    before(() => {
      cy.visit(`/${P_html}?${queriesFrom("xfa_imm5257e.pdf")}`);
    });

    it.skip("must search xfa correctly", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await page.click("#viewFind");
      //     await page.waitForSelector("#viewFind", { hidden: false });
