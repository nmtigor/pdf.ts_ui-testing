 ...
 *
 * @module pdf/pdf.ts-test/copy_paste.i.cy
 * @license Apache-2.0
 ******************************************************************************/
...
describe("Copy and paste", () => {
  describe("all text", () => {
    before(() => {
      cy.visit(`/${P_html}?${queriesFrom("tracemonkey.pdf", 100)}`);
      // await mockClipboard(pages);
    });

    it.skip("must check that we've all the contents on copy/paste", () => {
      // await Promise.all(
  ...
  describe("Copy/paste and ligatures", () => {
    before(() => {
      cy.visit(`/${P_html}?${queriesFrom("copy_paste_ligatures.pdf", 100)}`);
      // await mockClipboard(pages);
    });

    it.skip("must check that the ligatures have been removed when the text has been copied", () => {
      // await Promise.all(
      //   pages.map(async ([browserName, page]) => {
      //     await page.waitForSelector(
      //       ".page[data-page-number='1'] .textLayer .endOfContent",
